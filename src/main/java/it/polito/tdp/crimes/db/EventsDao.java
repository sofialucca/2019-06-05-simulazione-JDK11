...
public class EventsDao {
	...
	public District getMiglioreDistretto(Map<Integer, District> mappa, int anno) {
		String sql = "SELECT district_id "
				+ "FROM `events` "
				+ "WHERE YEAR(reported_date) = ? "
				+ "GROUP BY district_id "
				+ "ORDER BY COUNT(incident_id)";
		try {
			Connection conn = DBConnect.getConnection() ;
			...
			ResultSet res = st.executeQuery() ;
			res.next();
			District d = mappa.get(res.getInt("district_id"));
	...
	public List<Event> getEventiGiorno(int giorno, int mese, int anno) {
		String sql = "SELECT * "
				+ "FROM `events` "
